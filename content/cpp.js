      ...
      exercises: [
        {
          ...
          hints: [
            { zh: '想清楚「换行」由谁负责：每条输出语句末尾放一个换行标记，两句话自然分成两行。', en: 'Decide who is responsible for the line break: put a newline marker at the end of each output statement and the two sentences split naturally.' },
            { zh: '语法：\`std::cout << "..." << std::endl;\`，其中 \`std::endl\` 表示换行。', en: 'Syntax: \`std::cout << "..." << std::endl;\` where \`std::endl\` produces the newline.' },
            { zh: '写两条语句：第一条输出 \`Hello, World!\`，第二条输出 \`I am learning C++\`，各自末尾接 \`<< std::endl\`，标点照抄题目。', en: 'Write two statements: print \`Hello, World!\` first, then \`I am learning C++\`, each followed by \`<< std::endl\`; copy the punctuation exactly.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '*\n**\n***\n',
          hints: [
            { zh: '一共三行，每行星号数量递增；让每条输出语句负责一行即可。', en: 'Three lines in total, each with one more star; let each output statement handle one line.' },
            { zh: '语法：\`std::cout << "*" << std::endl;\`，双引号里放几个 \`*\` 就输出几个。', en: 'Syntax: \`std::cout << "*" << std::endl;\` — however many \`*\` you put between the quotes is how many get printed.' },
            { zh: '分三条语句，分别输出一个、两个、三个星号，每条末尾接 \`std::endl\`。', en: 'Use three statements printing one, two, and three stars respectively, each ending with \`std::endl\`.' },
          ],
          solution: '#include <iostream>\n\nint main() {\n    std::cout << "*" << std::endl;\n    std::cout << "**" << std::endl;\n    std::cout << "***" << std::endl;\n    return 0;\n}\n',
        },
      ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '28\n',
          hints: [
            { zh: '面积就是宽乘高。你需要一个新的 int 变量存结果，或者直接把乘法表达式交给 cout。', en: 'Area is width times height. Either store the result in a new int variable, or hand the multiplication expression straight to cout.' },
            { zh: '语法：\`int area = width * height;\` 之后输出 area；或直接 \`std::cout << width * height << std::endl;\`。', en: 'Syntax: \`int area = width * height;\` then print area; or directly \`std::cout << width * height << std::endl;\`.' },
            { zh: '定义 \`int area = width * height;\`，再输出 area 并换行。注意题目要求只输出这个整数，不要多余的说明文字。', en: 'Define \`int area = width * height;\`, then print area with a newline. The task wants only the integer — no extra explanatory text.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '3\n2\n1\n',
          hints: [
            { zh: '拆位的关键：取余 \`%\` 拿到当前最低位，整除 \`/\` 把数字往右「挪」一位。', en: 'The key idea: remainder \`%\` grabs the current lowest digit, integer division \`/\` shifts the number right by one digit.' },
            { zh: '语法：\`n % 10\` 得个位；\`(n / 10) % 10\` 得十位；\`n / 100\` 得百位。', en: 'Syntax: \`n % 10\` gives the ones digit; \`(n / 10) % 10\` the tens; \`n / 100\` the hundreds.' },
            { zh: '依次输出 \`n % 10\`、\`(n / 10) % 10\`、\`n / 100\`，每个一行。', en: 'Print \`n % 10\`, \`(n / 10) % 10\`, then \`n / 100\`, one per line.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '62\n55\n',
          hints: [
            { zh: '一小时 60 秒。想清楚「多少个整分钟」和「剩下多少秒」分别该用除法还是取余。', en: 'A minute is 60 seconds. Figure out whether "how many whole minutes" and "how many seconds left" call for division or remainder.' },
            { zh: '语法：\`total / 60\` 得整分钟数，\`total % 60\` 得剩余秒数。', en: 'Syntax: \`total / 60\` gives whole minutes; \`total % 60\` gives the leftover seconds.' },
            { zh: '两行输出：先 \`std::cout << (total / 60) << std::endl;\`，再输出 \`total % 60\`。', en: 'Two lines: first \`std::cout << (total / 60) << std::endl;\`, then print \`total % 60\`.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '3\n3.5\n3.5\n3.5\n',
          hints: [
            { zh: '这四个式子的区别只在于「有没有小数参与」。判断每一个式子两边是不是整数即可。', en: 'The four expressions differ only in "whether a floating-point number takes part". Check, for each one, whether both sides are integers.' },
            { zh: '语法：\`std::cout << (7 / 2) << std::endl;\`，其他同理；小数默认会打印出小数部分。', en: 'Syntax: \`std::cout << (7 / 2) << std::endl;\` and so on; doubles print their fractional part by default.' },
            { zh: '依次输出 \`(7 / 2)\`、\`(7.0 / 2)\`、\`(7 / 2.0)\`、\`(7.0 / 2.0)\`，各占一行；结果为 \`3\`、\`3.5\`、\`3.5\`、\`3.5\`。', en: 'Print \`(7 / 2)\`, \`(7.0 / 2)\`, \`(7 / 2.0)\`, \`(7.0 / 2.0)\` one per line; the results are \`3\`, \`3.5\`, \`3.5\`, \`3.5\`.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: 'B\n',
          hints: [
            { zh: '这是典型的「分段判断」：从最高档开始往下检查，命中一档就输出并结束。', en: 'This is a classic banded check: start from the top grade and work down; the first band that matches wins.' },
            { zh: '语法：\`if (score >= 90) {...} else if (score >= 80) {...} else {...}\`，每个分支里输出对应字母。', en: 'Syntax: \`if (score >= 90) {...} else if (score >= 80) {...} else {...}\`, printing the matching letter in each branch.' },
            { zh: '按 A、B、C、D、F 的顺序写五个分支，条件依次是 \`score >= 90\`、\`>= 80\`、\`>= 70\`、\`>= 60\`、else；输出后接 \`std::endl\`。', en: 'Write five branches in order A, B, C, D, F with conditions \`score >= 90\`, \`>= 80\`, \`>= 70\`, \`>= 60\`, else; end each output with \`std::endl\`.' },
          ],
          solution: '#include <iostream>\n\nint main() {\n    int score = 85;\n    if (score >= 90) {\n        std::cout << "A" << std::endl;\n    } else if (score >= 80) {\n        std::cout << "B" << std::endl;\n    } else if (score >= 70) {\n        std::cout << "C" << std::endl;\n    } else if (score >= 60) {\n        std::cout << "D" << std::endl;\n    } else {\n        std::cout << "F" << std::endl;\n    }\n    return 0;\n}\n',
        },
        {
          ...
          expectedOutput: '12\n',
          hints: [
            { zh: '最大值要不小于另外两个数。可以分三种情况讨论：a 最大、b 最大、c 最大。', en: 'The maximum is the one not smaller than the other two. Consider three cases: a is the max, b is the max, or c is the max.' },
            { zh: '语法：\`if (a >= b && a >= c) {...} else if (b >= a && b >= c) {...} else {...}\`，\`&&\` 表示两个条件同时成立。', en: 'Syntax: \`if (a >= b && a >= c) {...} else if (b >= a && b >= c) {...} else {...}\`; \`&&\` means both conditions hold.' },
            { zh: '三个分支分别输出 a、b、c；最后兜底 else 就是 c 最大。只输出数字本身，不带其他文字。', en: 'The three branches print a, b, and c respectively; the final else covers c being the max. Print only the number, nothing else.' },
          ],
          solution: '#include <iostream>\n\nint main() {\n    int a = 12, b = 7, c = 9;\n    if (a >= b && a >= c) {\n        std::cout << a << std::endl;\n    } else if (b >= a && b >= c) {\n        std::cout << b << std::endl;\n    } else {\n        std::cout << c << std::endl;\n    }\n    return 0;\n}\n',
        },
      ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '5050\n',
          hints: [
            { zh: '准备一个累加器从 0 开始，循环里每次把当前的数加进去；从 1 循环到 100。', en: 'Prepare an accumulator starting at 0 and add the current number on every round; loop from 1 to 100.' },
            { zh: '语法：\`for (int i = 1; i <= 100; i++) { total += i; }\`，其中 \`+=\` 是「加到自己身上」。', en: 'Syntax: \`for (int i = 1; i <= 100; i++) { total += i; }\`, where \`+=\` means "add to myself".' },
            { zh: 'total 初始为 0；循环结束后 \`std::cout << total << std::endl;\` 输出一次即可。', en: 'Start total at 0; after the loop, \`std::cout << total << std::endl;\` prints it exactly once.' },
          ],
        ...
        },
        {
          ...
          hints: [
            { zh: '每一行的形状都是 \`7 x i = 结果\`，只有 i 和结果在变——正是循环的用武之地。', en: 'Every line has the shape \`7 x i = result\`; only i and the result change — a perfect job for a loop.' },
            { zh: '语法：\`std::cout << "7 x " << i << " = " << (7 * i) << std::endl;\`，字符串和数字可以交替串联输出。', en: 'Syntax: \`std::cout << "7 x " << i << " = " << (7 * i) << std::endl;\` — strings and numbers can be chained alternately.' },
            { zh: '写一个 i 从 1 到 9 的 for 循环，循环体里输出上面那行内容；注意空格位置必须与题目完全一致。', en: 'Write a for loop with i from 1 to 9 whose body prints the line above; the spaces must match the required format exactly.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '2\n3\n5\n7\n11\n13\n17\n19\n',
          hints: [
            { zh: '素数的定义：大于 1 且只能被 1 和自身整除。先想好怎么判定「能被整除」，再想循环范围。', en: 'A prime is a number greater than 1 divisible only by 1 and itself. First decide how to test "divisible", then the loop range.' },
            { zh: '语法：\`if (n % i == 0) return false;\` 检测到任一因子立即返回 false；循环用 \`for (int i = 2; i < n; i++)\`。', en: 'Syntax: \`if (n % i == 0) return false;\` bails out on any factor; loop with \`for (int i = 2; i < n; i++)\`.' },
            { zh: 'is_prime 里先处理 \`n < 2\` 返回 false；再循环找因子。main 里从 2 到 19 逐个调用，是素数就输出该数字本身。', en: 'In is_prime, return false when \`n < 2\`, then loop for factors. In main, test 2 through 19 and print each number that passes.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '8\n3\n',
          hints: [
            { zh: '函数参数写成 \`int&\` 才能改到调用方的变量；交换本身需要一个临时变量中转。', en: 'Parameters must be \`int&\` to affect the variables of the caller; the swap itself needs a temporary variable as a relay.' },
            { zh: '语法：\`void swap(int& a, int& b)\`，体内三步：\`int t = a;\` \`a = b;\` \`b = t;\`。', en: 'Syntax: \`void swap(int& a, int& b)\` with three steps in the body: \`int t = a;\` \`a = b;\` \`b = t;\`.' },
            { zh: '实现 swap 后在 main 里调用 \`swap(a, b)\`，然后分两行输出 a 和 b。', en: 'After implementing swap, call \`swap(a, b)\` in main, then print a and b on two lines.' },
          ],
          solution: '#include <iostream>\n\nvoid swap(int& a, int& b) {\n    int t = a;\n    a = b;\n    b = t;\n}\n\nint main() {\n    int a = 3, b = 8;\n    swap(a, b);\n    std::cout << a << std::endl;\n    std::cout << b << std::endl;\n    return 0;\n}\n',
        },
      ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '108\n42\n4\n6\n',
          hints: [
            { zh: '四个量里，总和用累加器循环求；最值可以先假设第一个元素最大最小再逐个比较；个数直接问 vector。', en: 'Of the four quantities: sum via an accumulator loop; max/min by assuming the first element then comparing one by one; the count by asking the vector.' },
            { zh: '语法：\`int total = 0; for (int x : nums) total += x;\`；\`std::max_element\` / \`std::min_element\` 返回迭代器，要用 \`*\` 取值；\`nums.size()\` 是个数。', en: 'Syntax: \`int total = 0; for (int x : nums) total += x;\`; \`std::max_element\` / \`std::min_element\` return iterators — dereference with \`*\`; \`nums.size()\` is the count.' },
            { zh: '依次输出累加结果、\`*std::max_element(nums.begin(), nums.end())\`、\`*std::min_element(...)\`、\`nums.size()\`，各占一行。', en: 'Print the sum, then \`*std::max_element(nums.begin(), nums.end())\`, then \`*std::min_element(...)\`, then \`nums.size()\`, one per line.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '4\n16\n36\n64\n100\n',
          hints: [
            { zh: '两步走：先筛选（1 到 10 里哪些是偶数），再把它们的平方存进 vector，最后统一输出。', en: 'Two phases: filter (which of 1 to 10 are even), store their squares into the vector, then print them all.' },
            { zh: '语法：\`if (i % 2 == 0) result.push_back(i * i);\`，偶数判定用取余；\`push_back\` 往尾部追加。', en: 'Syntax: \`if (i % 2 == 0) result.push_back(i * i);\` — test evenness with remainder; \`push_back\` appends.' },
            { zh: '先循环 1..10 把偶数平方 push_back 进 result，再范围 for 逐行输出 result 里的每个元素。', en: 'Loop 1..10 pushing squares of evens into result, then range-for over result printing one per line.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '9\nHELLO C++\nH +\n',
          hints: [
            { zh: '三样输出：长度直接问 string；大写形式可以先复制一份再逐字符改；首尾字符就是下标 0 和最后一个。', en: 'Three outputs: the length by asking the string; uppercase by copying and converting character by character; the first and last are index 0 and the final one.' },
            { zh: '语法：\`s.size()\`；\`s[i] = std::toupper(s[i])\`（需 \`<cctype>\`）；\`s[0]\` 与 \`s.back()\`。', en: 'Syntax: \`s.size()\`; \`s[i] = std::toupper(s[i])\` (needs \`<cctype>\`); \`s[0]\` and \`s.back()\`.' },
            { zh: '复制 \`std::string upper = s;\`，循环把每个字符 toupper；输出三行：长度、upper、\`s[0] << " " << s.back()\`。', en: 'Copy with \`std::string upper = s;\`, toupper every character in a loop; print three lines: length, upper, then \`s[0] << " " << s.back()\`.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '1\n0\n',
          hints: [
            { zh: '回文就是「正着读反着读一样」。两头各派一个指针往中间走，边走边比。', en: 'A palindrome reads the same forwards and backwards. Send one pointer from each end toward the middle, comparing as they go.' },
            { zh: '语法：\`size_t i = 0, j = s.size() - 1;\`，循环 \`while (i < j)\`；比较前用 \`std::tolower\` 统一大小写；\`i++\`、\`j--\`。', en: 'Syntax: \`size_t i = 0, j = s.size() - 1;\` with \`while (i < j)\`; lowercase with \`std::tolower\` before comparing; then \`i++\`, \`j--\`.' },
            { zh: '遇到一对不相等立刻 return false；指针相遇仍没发现不等就 return true。注意参数是 const 引用，不能修改原串。', en: 'Return false immediately on a mismatch; if the pointers meet with no mismatch, return true. The parameter is a const reference — do not modify the original.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '8 3\n',
          hints: [
            { zh: '想让函数改到 main 里的变量，参数必须「引用」而不是普通值；交换需要临时变量中转。', en: 'For the function to reach the variables in main, the parameters must be "references" rather than plain values; the swap needs a temporary variable.' },
            { zh: '语法：\`void mySwap(int& a, int& b)\`，体内三步：\`int t = a; a = b; b = t;\`。', en: 'Syntax: \`void mySwap(int& a, int& b)\` with the three steps: \`int t = a; a = b; b = t;\`.' },
            { zh: '调用 \`mySwap(a, b);\` 后分一行输出 a 和 b，中间用空格：\`std::cout << a << " " << b << std::endl;\`。', en: 'After calling \`mySwap(a, b);\` print a and b on one line separated by a space: \`std::cout << a << " " << b << std::endl;\`.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '1\n4\n9\n16\n25\n',
          hints: [
            { zh: '流程是四步：申请内存 → 填数据 → 输出 → 归还内存。数组元素下标 0..4 对应 1..5。', en: 'The flow is four steps: allocate -> fill -> print -> free. Indices 0..4 correspond to values 1..5.' },
            { zh: '语法：\`int* arr = new int[5];\`，循环填 \`arr[i] = (i + 1) * (i + 1);\`，最后 \`delete[] arr;\`。', en: 'Syntax: \`int* arr = new int[5];\`, fill with \`arr[i] = (i + 1) * (i + 1);\` in a loop, then \`delete[] arr;\`.' },
            { zh: '两个 for 循环：第一个把每个下标的平方填进去，第二个逐行输出；输出完成后再 delete[]。', en: 'Two for loops: the first fills each slot with its square, the second prints line by line; call delete[] only after printing.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '12\n14\n',
          hints: [
            { zh: '类的三件套：私有成员存宽高、构造函数负责初始化、两个成员函数分别算面积和周长。', en: 'The class needs three parts: private members for width/height, a constructor for initialization, and two member functions for area and perimeter.' },
            { zh: '语法：\`Rectangle(int w, int h) : width(w), height(h) {}\`；成员函数 \`int area() const { return width * height; }\`。', en: 'Syntax: \`Rectangle(int w, int h) : width(w), height(h) {}\`; member function \`int area() const { return width * height; }\`.' },
            { zh: '周长是 \`2 * (width + height)\`。创建 \`Rectangle r(3, 4);\` 后分两行输出 \`r.area()\` 与 \`r.perimeter()\`。', en: 'Perimeter is \`2 * (width + height)\`. After creating \`Rectangle r(3, 4);\`, print \`r.area()\` and \`r.perimeter()\` on two lines.' },
          ],
        ...
        },
        {
          ...
          hints: [
            { zh: '先定义带 virtual speak 的基类，再定义重写它的 Dog；用基类指针数组统一管理两种对象，循环里调用 speak 就是多态。', en: 'First define the base with a virtual speak, then Dog overriding it; store both in a base-pointer array, and calling speak in a loop is polymorphism.' },
            { zh: '语法：\`virtual std::string speak() const;\`、\`std::string speak() const override\`、派生构造 \`Dog(std::string n) : Animal(n) {}\`。', en: 'Syntax: \`virtual std::string speak() const;\`, \`std::string speak() const override\`, derived constructor \`Dog(std::string n) : Animal(n) {}\`.' },
            { zh: '基类析构写 \`virtual ~Animal() = default;\`；main 里 \`Animal* pets[] = { new Animal("Cat"), new Dog("Rex") };\`，循环输出 \`p->speak()\` 并 \`delete p\`。', en: 'Give the base \`virtual ~Animal() = default;\`; in main use \`Animal* pets[] = { new Animal("Cat"), new Dog("Rex") };\`, loop printing \`p->speak()\` and \`delete p\`.' },
          ],
          solution: '#include <iostream>\n#include <string>\n\nclass Animal {\npublic:\n    Animal(std::string n) : name(n) {}\n    virtual std::string speak() const { return name + " makes a sound"; }\n    virtual ~Animal() = default;\nprotected:\n    std::string name;\n};\n\nclass Dog : public Animal {\npublic:\n    Dog(std::string n) : Animal(n) {}\n    std::string speak() const override { return name + " barks"; }\n};\n\nint main() {\n    Animal* pets[] = { new Animal("Cat"), new Dog("Rex") };\n    for (Animal* p : pets) {\n        std::cout << p->speak() << std::endl;\n        delete p;\n    }\n    return 0;\n}\n',
        },
      ],
      ...
      exercises: [
        {
          ...
          hints: [
            { zh: '两个阶段：先写入，再读回。读的时候准备一个行数计数器，在 getline 循环里累加。', en: 'Two phases: write first, then read back. While reading, keep a line counter incremented inside the getline loop.' },
            { zh: '语法：\`std::ofstream out("test.txt"); out << "Hello File";\`；读回用 \`while (std::getline(in, line)) lines++;\`。', en: 'Syntax: \`std::ofstream out("test.txt"); out << "Hello File";\`; read back with \`while (std::getline(in, line)) lines++;\`.' },
            { zh: '写入时不用 endl（保持 1 行）；读回循环里第一次把内容存进变量、每轮 lines 加 1；最后输出内容和 \`"lines: " << lines\`。', en: 'Write without endl so it stays one line; in the read loop save the content on the first round and increment lines each round; finally print the content and \`"lines: " << lines\`.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '3\n17\n',
          hints: [
            { zh: '写入三行后逐行读回；两个统计量（行数、字符数）都能在同一个 getline 循环里完成。', en: 'Write three lines, read them back; both statistics (line count, character count) can be gathered in the same getline loop.' },
            { zh: '语法：循环里 \`lines++;\` 且 \`chars += line.size();\`（\`size()\` 不含换行符，正好符合题意）。', en: 'Syntax: inside the loop, \`lines++;\` and \`chars += line.size();\` (\`size()\` excludes the newline, exactly as required).' },
            { zh: '写入三个水果各带一次 endl；读回循环同时累加两个计数；最后分两行输出 lines 和 chars。', en: 'Write the three fruits each followed by endl; accumulate both counters in the read loop; print lines then chars on two lines.' },
          ],
