      ...
      exercises: [
        {
          ...
          hints: [
            { zh: '程序要「说一句话」：在 main 方法体内写一条打印语句，把想说的内容原样放进双引号里。', en: 'The program needs to say something: write one print statement inside main and put the exact text in double quotes.' },
            { zh: 'System.out.println("内容"); —— System 的 S 大写，内容放在英文双引号里，句末加分号。', en: 'System.out.println("text"); — capital S in System, text in double quotes, semicolon at the end.' },
            { zh: '在 main 的大括号内写一行：调用 System.out.println，参数是双引号包住的 Hello, World!，别忘分号。', en: 'Inside the braces of main write one line: call System.out.println with the double-quoted text Hello, World!, then add the semicolon.' },
          ],
        ...
        },
        {
          ...
          hints: [
            { zh: '两行输出就是两条打印语句，println 每次打完会自动换行。', en: 'Two lines of output means two print statements; each println breaks the line automatically.' },
            { zh: '写两条 System.out.println("..."); 语句，一条一行，先后顺序就是输出顺序。', en: 'Write two System.out.println("..."); statements, one per line, in the same order as the output.' },
            { zh: '第一条 println 参数是 "Hello, Java"，第二条是 "I am learning"，大小写与标点要完全一致。', en: 'The first println takes "Hello, Java", the second takes "I am learning"; match capitalization and punctuation exactly.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '28\n',
          hints: [
            { zh: '面积 = 宽 × 高。两个变量已经造好，你只需要算出结果并打印。', en: 'Area = width times height. Both variables are ready; you only need to compute and print.' },
            { zh: 'System.out.println(表达式) 可以直接打印计算结果，乘号是 *。', en: 'System.out.println(expression) prints the computed result directly; the multiplication sign is *.' },
            { zh: '在 main 里写一行 System.out.println(width * height);，输出 28。', en: 'Write System.out.println(width * height); in main, which prints 28.' },
          ],
          solution: 'public class Main {\n    public static void main(String[] args) {\n        int width = 7;\n        int height = 4;\n        System.out.println(width * height);\n    }\n}\n',
        },
        {
          ...
          expectedOutput: '2030\n',
          hints: [
            { zh: '“2026”现在是文字，不能直接加 4；要先把文字转成数字。', en: 'The value is text right now and cannot be added to 4; convert it to a number first.' },
            { zh: 'Integer.parseInt(s) 把字符串转成 int；String.valueOf 则是反方向。', en: 'Integer.parseInt(s) turns a String into an int; String.valueOf goes the other way.' },
            { zh: '定义 int n = Integer.parseInt(s); 然后输出 n + 4。', en: 'Define int n = Integer.parseInt(s); then print n + 4.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '62\n55\n',
          hints: [
            { zh: '一分钟 60 秒：总秒数除以 60 得完整分钟数，除不尽的部分就是剩余秒数。', en: 'There are 60 seconds in a minute: total divided by 60 gives whole minutes, and the leftover is the remaining seconds.' },
            { zh: '整数除法 total / 60 得分钟数；取余 total % 60 得剩余秒数，结果都是 int。', en: 'Integer division total / 60 gives minutes; total % 60 gives the remaining seconds; both are int.' },
            { zh: '两行输出：先 System.out.println(total / 60); 再 System.out.println(total % 60);', en: 'Two lines of output: first System.out.println(total / 60); then System.out.println(total % 60);' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '5\n7\n',
          hints: [
            { zh: '关键是「先后顺序」：x++ 先把旧值交出去再加 1；++x 先加 1 再交出去。', en: 'The key is order: x++ hands out the old value before adding 1; ++x adds 1 first and then hands out the new value.' },
            { zh: '两条 System.out.println 分别打印 x++ 和 ++x，打印的是表达式的值。', en: 'Two System.out.println statements print x++ and ++x respectively — each prints the value of the expression.' },
            { zh: '第一行打印 x++ 得 5，此后 x 是 6；第二行 ++x 先变 7 再打印 7。', en: 'The first line prints x++ which yields 5, leaving x at 6; the second line ++x raises it to 7 then prints 7.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: 'B\n',
          hints: [
            { zh: '多个分数段对应多个等级，从最高档开始往下判断，命中就输出并停止。', en: 'Map each score band to a grade; check from the highest band downward, print on a hit and stop.' },
            { zh: '用 if / else if / else 链，条件形如 score >= 90、score >= 80，依次降低。', en: 'Use an if / else if / else chain with conditions like score >= 90, score >= 80, in decreasing order.' },
            { zh: '按 90/80/70/60 依次写 else if，每个分支 System.out.println 相应字母，最后的 else 打印 F。', en: 'Write else if branches for 90/80/70/60; each prints its letter, and the final else prints F.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: 'Thu\n',
          hints: [
            { zh: 'day 的取值固定且只有几种可能，正好适合 switch 逐个匹配。', en: 'day takes one of a few fixed values — a perfect fit for switch matching.' },
            { zh: 'switch (day) 里每个 case 值后跟要执行的语句和 break，default 处理其余情况。', en: 'In switch (day), each case value is followed by its statement and a break; default covers everything else.' },
            { zh: '写 case 1 到 case 4 分别打印 Mon/Tue/Wed/Thu 并各加 break，default 打印 Other。', en: 'Write case 1 through case 4 printing Mon/Tue/Wed/Thu each with a break, and default printing Other.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '5050\n',
          hints: [
            { zh: '一口气算不出来？准备一个累加器从 0 开始，让循环每圈往里加一个数。', en: 'Cannot compute it in one breath? Prepare an accumulator starting at 0 and let the loop add one number per lap.' },
            { zh: '先 int sum = 0; 再 for (int i = 1; i <= 100; i++)，循环体里写 sum += i;。', en: 'First int sum = 0; then for (int i = 1; i <= 100; i++) with sum += i; inside.' },
            { zh: '循环结束后用 System.out.println(sum); 输出，结果是 5050，不要用公式直接算。', en: 'After the loop, print with System.out.println(sum); the result is 5050 — do not use the closed-form formula.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '1\n3\n5\n7\n9\n',
          hints: [
            { zh: '奇数就是除以 2 余 1 的数；循环里判断一下，是偶数就跳过。', en: 'An odd number leaves a remainder of 1 when divided by 2; test inside the loop and skip the evens.' },
            { zh: 'for (int i = 1; i <= 10; i++) 里先写 if (i % 2 == 0) continue; 再打印。', en: 'Inside for (int i = 1; i <= 10; i++), first check if (i % 2 == 0) continue; then print.' },
            { zh: 'continue 之后的 println 每轮只对奇数执行，输出 1、3、5、7、9 各占一行。', en: 'The println after continue runs only for odd numbers, printing 1, 3, 5, 7, 9 one per line.' },
          ],
          solution: 'public class Main {\n    public static void main(String[] args) {\n        for (int i = 1; i <= 10; i++) {\n            if (i % 2 == 0) continue;\n            System.out.println(i);\n        }\n    }\n}\n',
        },
      ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '15\n',
          hints: [
            { zh: '题目要求两件事：定义一个会求和的方法，以及在 main 里用它并打印结果。', en: 'Two tasks here: define a method that adds two numbers, and call it from main to print the result.' },
            { zh: '方法签名照题目写：public static int sum(int a, int b)，方法体 return a + b;，写在类里、main 外。', en: 'Use the required signature: public static int sum(int a, int b) with body return a + b;, placed in the class outside main.' },
            { zh: 'main 里写 System.out.println(sum(7, 8));，返回值 15 直接被打印。', en: 'In main write System.out.println(sum(7, 8)); the return value 15 gets printed directly.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '9\n9\n',
          hints: [
            { zh: '两个方法同名但参数个数不同，Java 会按你传的参数个数自动选。', en: 'The two methods share a name but differ in parameter count; Java picks automatically by how many arguments you pass.' },
            { zh: '两个参数版：public static void printMax(int a, int b)，用 a > b ? a : b 求最大并 println。', en: 'Two-arg version: public static void printMax(int a, int b); compute a > b ? a : b and println it.' },
            { zh: '三参数版：设 int m = a; 若 b > m 则 m = b; 若 c > m 则 m = c; 最后 println(m)。main 里依次调用两个版本。', en: 'Three-arg version: set int m = a; if b > m set m = b; if c > m set m = c; finally println(m). Call both versions in main in order.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '108\n42\n4\n6\n',
          hints: [
            { zh: '四项统计可以一次循环同时完成：准备 sum、max、min 三个「记录本」，边走边记。', en: 'All four statistics can come from one pass: keep sum, max and min as running records while you walk the array.' },
            { zh: 'sum 从 0 加起；max/min 先设为 nums[0]，循环里遇到更大的就更新 max、更小的更新 min；长度是 nums.length。', en: 'Start sum at 0; initialize max/min to nums[0], update them when a larger or smaller element appears; the length is nums.length.' },
            { zh: '用增强 for 遍历：sum += n; if (n > max) max = n; if (n < min) min = n; 循环后按顺序 println sum、max、min、nums.length。', en: 'With an enhanced for: sum += n; if (n > max) max = n; if (n < min) min = n; after the loop println sum, max, min, nums.length in order.' },
          ],
        ...
        },
        {
          ...
          hints: [
            { zh: '排序和格式化打印都有现成工具，不需要自己写循环。', en: 'Sorting and formatting both have ready-made tools; no hand-written loop is needed.' },
            { zh: 'Arrays.sort(a) 原地排序；Arrays.toString(a) 返回 [1, 2, 3] 格式的字符串；别忘 import java.util.Arrays;。', en: 'Arrays.sort(a) sorts in place; Arrays.toString(a) returns a string like [1, 2, 3]; remember import java.util.Arrays;.' },
            { zh: '两行：Arrays.sort(a); 然后 System.out.println(Arrays.toString(a));。', en: 'Two lines: Arrays.sort(a); then System.out.println(Arrays.toString(a));' },
          ],
      ...
      exercises: [
        {
          ...
          hints: [
            { zh: '两个 new 出来的字符串是两个独立对象，但内容相同——两种比较自然给出不同答案。', en: 'Two strings created with new are two independent objects with equal content — the two kinds of comparison naturally disagree.' },
            { zh: 'System.out.println(a == b); 和 System.out.println(a.equals(b)); 各打印一个 boolean。', en: 'System.out.println(a == b); and System.out.println(a.equals(b)); each print a boolean.' },
            { zh: '依次输出两行：== 比较地址得到 false，equals 比较内容得到 true。', en: 'Print two lines in order: == compares references and yields false, equals compares content and yields true.' },
          ],
        ...
        },
        {
          ...
          hints: [
            { zh: 'String 自身不可变，想「加工」文字可以借助 StringBuilder。', en: 'String itself is immutable; use a StringBuilder when you need to transform text.' },
            { zh: 'new StringBuilder(s) 用 s 初始化，reverse() 反转，toString() 变回 String。', en: 'new StringBuilder(s) initializes from s, reverse() flips it, toString() converts back to String.' },
            { zh: '一行搞定：System.out.println(new StringBuilder(s).reverse().toString()); 输出 olleh。', en: 'One line does it: System.out.println(new StringBuilder(s).reverse().toString()); prints olleh.' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '99\n',
          hints: [
            { zh: 'b = a 不是复制数组，而是让两个变量指向同一个数组，所以改 b 就是改 a。', en: 'b = a does not copy the array; both variables now point at the same one, so changing b changes a.' },
            { zh: '直接用 System.out.println(a[0]); 输出修改后的值。', en: 'Simply use System.out.println(a[0]); to print the modified value.' },
            { zh: '在 TODO 处写 System.out.println(a[0]);，因为 b 和 a 共享同一个数组，结果为 99。', en: 'At the TODO write System.out.println(a[0]); since b and a share the array, it prints 99.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '-1\n',
          hints: [
            { zh: 's 可能没有指向任何对象，直接调 s.length() 会抛异常，先判断再使用。', en: 's may point at no object at all; calling s.length() directly throws, so test before using.' },
            { zh: 'if (s != null) 时打印 s.length()，否则打印 -1，两个分支各一条 println。', en: 'When s != null print s.length(), otherwise print -1 — one println per branch.' },
            { zh: '写 if (s != null) { System.out.println(s.length()); } else { System.out.println(-1); }。', en: 'Write if (s != null) { System.out.println(s.length()); } else { System.out.println(-1); }' },
          ],
      ...
      exercises: [
        {
          ...
          expectedOutput: '12\n14\n',
          hints: [
            { zh: '按题目要求把「图纸」写全：两个字段、一个构造器、两个方法，然后在 main 里造对象并调用。', en: 'Write the blueprint completely as required: two fields, one constructor, two methods, then create an object in main and call them.' },
            { zh: 'class Rectangle 里定义 private int width 与 private int height；构造器里 this.width = width;；area 返回乘积，perimeter 返回 2 * (width + height)。', en: 'In class Rectangle define private int width and private int height; assign this.width = width; in the constructor; area returns the product, perimeter returns 2 * (width + height).' },
            { zh: 'Rectangle 写成不带 public 的类放在 Main 后面；main 里 new Rectangle(3, 4) 后依次 println(r.area()) 与 r.perimeter()。', en: 'Declare Rectangle without public after Main; in main create new Rectangle(3, 4) then println(r.area()) and println(r.perimeter()) in order.' },
          ],
        ...
        },
        {
          ...
          hints: [
            { zh: '父类管公共部分（名字），子类用 extends 继承并改写叫声；用父类类型接住子类对象体会多态。', en: 'The parent holds what is common (the name); the child extends it and rewrites speak; catch the child object with a parent-typed variable to see polymorphism.' },
            { zh: 'Animal 定义 protected String name、构造器和 speak；Dog extends Animal，构造器第一行 super(name)，speak 上加 @Override。', en: 'Animal defines protected String name, a constructor and speak; Dog extends Animal, its constructor starts with super(name), and speak is annotated @Override.' },
            { zh: 'main 里写 Animal a = new Dog("Rex"); System.out.println(a.speak()); 输出 Rex barks。', en: 'In main write Animal a = new Dog("Rex"); System.out.println(a.speak()); which prints Rex barks.' },
          ],
      ...
      exercises: [
        {
          ...
          hints: [
            { zh: '两步走：先把文字写进 test.txt，再把它读回来打印。', en: 'Two steps: write the text into test.txt, then read it back and print it.' },
            { zh: 'try (PrintWriter pw = new PrintWriter("test.txt")) 里用 pw.print("Hello File");（不带换行），再用 Files.readString(Paths.get("test.txt")) 读回。', en: 'Inside try (PrintWriter pw = new PrintWriter("test.txt")) use pw.print("Hello File"); (no newline), then read back with Files.readString(Paths.get("test.txt")).' },
            { zh: '把读回的字符串交给 System.out.println(content); 输出，内容正是 Hello File。', en: 'Hand the read-back string to System.out.println(content); it is exactly Hello File.' },
          ],
        ...
        },
        {
          ...
          expectedOutput: '5\n6\n6\n',
          hints: [
            { zh: '先写三行水果名，再逐行读回；输出的不是水果本身，而是每行的长度。', en: 'First write three fruit lines, then read them back; print each line length, not the fruit itself.' },
            { zh: '用 Files.writeString(Paths.get("fruits.txt"), "apple\\nbanana\\ncherry\\n") 写入；readAllLines 返回的 List 里换行符已被去掉。', en: 'Write with Files.writeString(Paths.get("fruits.txt"), "apple\\nbanana\\ncherry\\n"); readAllLines returns a List with line breaks already stripped.' },
            { zh: '用增强 for 遍历 lines，每行 System.out.println(line.length()); 输出 5、6、6。', en: 'Loop over lines with an enhanced for and System.out.println(line.length()); printing 5, 6, 6.' },
          ],
